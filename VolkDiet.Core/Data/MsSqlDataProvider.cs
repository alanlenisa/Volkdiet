...
namespace VolkDiet.Core.Data
{
    public class MsSqlDataProvider : IDataProvider
    {
        ...
        public void InitDb()
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var query = "IF OBJECT_ID(N'[__EFMigrationsHistory]') IS NULL" +
 " BEGIN" +
 ...
 "     CONSTRAINT [FK_Cateries_Libraries] FOREIGN KEY ([LIB_ID]) REFERENCES [Libraries] ([ID])" +
 "   );" +
 " END;" +
 " " +
 " " +
 ...
 "     CONSTRAINT [FK_Aliments_Cateries] FOREIGN KEY ([CAT_ID]) REFERENCES [Cateries] ([ID])," +
 ...
 " IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20220719082417_InitialDbMsSql')" +
 " BEGIN" +
 "   CREATE INDEX [IX_Cateries_LIB_ID] ON [Cateries] ([LIB_ID]);" +
 " END;" +
 " " +
 " " +
 " IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20220719082417_InitialDbMsSql')" +
